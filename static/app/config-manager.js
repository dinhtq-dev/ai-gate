...
function showChangePasswordModal() {
        ...
        <div class="modal-content" style="max-width: 450px;">
            <div class="modal-header">
                <h3>
                    ...
                    <span data-i18n="config.handoff.changePasswordTitle">${t('config.handoff.changePasswordTitle')}</span>
                </h3>
                <button class="modal-close" type="button" aria-label="${t('common.close')}">
            ...
            <div class="modal-body" style="padding: 1.5rem;">
                <form id="handoffPasswordForm" onsubmit="return false;">
                    <div class="form-group" style="margin-bottom: 1.25rem;">
                        <label for="handoffNewPassword" style="display: block; margin-bottom: 0.5rem; font-weight: 600; font-size: 0.9rem;" data-i18n="config.advanced.adminPassword">${t('config.advanced.adminPassword')}</label>
                    ...
                    <div class="form-group" style="margin-bottom: 1rem;">
                        <label for="handoffConfirmPassword" style="display: block; margin-bottom: 0.5rem; font-weight: 600; font-size: 0.9rem;" data-i18n="config.handoff.confirmPassword">${t('config.handoff.confirmPassword')}</label>
                        <div class="password-input-wrapper" style="position: relative; display: flex; align-items: center;">
                    ...
                    <small class="form-text" style="color: var(--text-secondary); display: block; margin-bottom: 1rem;" data-i18n="config.handoff.passwordHelpText">
                        <i class="fas fa-info-circle"></i> ${t('config.handoff.passwordHelpText')}
                    </small>
                </form>
            </div>
            <div class="modal-footer" style="display: flex; gap: 1rem; justify-content: flex-end; padding: 1rem 1.5rem; background: var(--bg-secondary); border-top: 1px solid var(--border-color); border-radius: 0 0 var(--radius-xl) var(--radius-xl);">
                <button type="button" class="btn btn-secondary cancel-btn" data-i18n="common.cancel">${t('common.cancel')}</button>
                <button type="button" class="btn btn-danger confirm-btn">
                    <i class="fas fa-save"></i> <span data-i18n="common.confirm">${t('common.confirm')}</span>
                </button>
            </div>
        </div>
    `;
    ...
    confirmBtn.addEventListener('click', async () => {
        ...
        if (!newPassword) {
            showToast(t('common.error'), t('common.passwordEmpty'), 'error');
            newPasswordEl.focus();
            return;
        }

        if (newPassword.length < 12) {
            showToast(t('common.error'), t('common.passwordTooShort'), 'error');
            newPasswordEl.focus();
            return;
        }

        if (newPassword !== confirmPassword) {
            showToast(t('common.error'), t('config.handoff.passwordMismatch'), 'error');
        ...
        try {
            ...
            confirmBtn.innerHTML = `<i class="fas fa-spinner fa-spin"></i> <span data-i18n="config.handoff.saving">${t('config.handoff.saving')}</span>`;
            
            await window.apiClient.post('/admin-password', { password: newPassword });
            
            showToast(t('common.success'), t('config.handoff.passwordUpdated'), 'success');
            ...
            confirmBtn.innerHTML = `<i class="fas fa-save"></i> <span data-i18n="common.confirm">${t('common.confirm')}</span>`;
        }
    });
...
function renderProviderTags(container, configs, isRequired) {
    ...
    container.innerHTML = visibleConfigs.map(c => `
        <button type="button" class="provider-tag" data-value="${escHtml(c.id)}">
            ...
            ${isModelProviderSelect ? `<span class="tag-pin-icon" title="${t('config.pin')}"><i class="fas fa-thumbtack"></i></span>` : ''}
        </button>
    `).join('');
...
async function saveConfiguration(options = {}) {
    ...
    if (fallbackChainValue) {
        try {
            ...
            showToast(t('common.error'), t('config.advanced.fallbackChainInvalid'), 'error');
            return;
        }
    } else {
    ...
    if (modelFallbackMappingValue) {
        try {
            ...
            showToast(t('common.error'), t('config.advanced.modelFallbackMappingInvalid'), 'error');
            return;
        }
    } else {
...
function generateApiKey() {
    ...
    copyToClipboard(randomKey).then(success => {
        if (success) {
            showToast(t('common.success'), t('config.apiKey.generatedAndCopied'), 'success');
        } else {
            showToast(t('common.success'), t('config.apiKey.generated'), 'success');
        }
    });
