...
function createConfigItemElement(config, index) {
    ...
    if (config.expiresAt) {
        ...
        let timeStr = '';
        if (isExpired) {
            timeStr = `<span class="expiration-tag expired">${t('upload.expiration.expired')}</span>`;
        } else {
            // 计算剩余秒数
            ...
            let duration = '';
            if (days > 0) duration += `${days}${t('common.date.days')}`;
            if (hours > 0 || days > 0) duration += `${hours}${t('common.date.hours')}`;
            duration += `${minutes}${t('common.date.minutes')}`;
            
            timeStr = `<span class="expiration-tag ${remainingSeconds < 3600 * 24 ? 'warning' : 'healthy'}">
                <i class="fas fa-clock"></i> ${duration} ${t('upload.expiration.remaining')}
            </span>`;
        }
        
        expirationHtml = `<div class="config-expiration-info">
            <span class="meta-item" title="${t('upload.detail.expiresAt')}: ${formatDate(config.expiresAt)}">
                ${timeStr}
            </span>
        </div>`;
    }
    ...
    const quickLinkBtnHtml = canQuickLink ?
        `<button class="btn-quick-link-main" data-path="${config.path}" title="${t('upload.quickLinkTitle', { name: providerInfo.displayName })}">
    ...
    const forceExpireBtnHtml = canForceExpire ?
        `<button class="btn-small btn-force-expire" data-path="${config.path}">
            <i class="fas fa-sync"></i> <span data-i18n="upload.action.forceExpire">${t('upload.action.forceExpire')}</span>
        </button>` : '';
    ...
    if (pathValueEl) {
        ...
        pathValueEl.title = t('models.clickToCopy');
    }
...
function generateUsageInfoHtml(config) {
    ...
    const typeLabel = typeLabels[usageType] || t('common.unknown');

    let detailsHtml = '';
    usageDetails.forEach(detail => {
        const isMain = detail.type === 'Main Config';
...
async function forceExpireConfig(path) {
    if (!confirm(t('upload.forceExpire.confirm'))) {
        return;
    }

    try {
        const result = await window.apiClient.post(`/upload-configs/force-expire/${encodeURIComponent(path)}`);
        showToast(t('common.success'), t('upload.forceExpire.success'), 'success');
        
        // 重新加载列表
        await loadConfigList();
    } catch (error) {
        console.error('强制过期失败:', error);
        showToast(t('common.error'), (t('upload.action.forceExpire.failed')) + ': ' + error.message, 'error');
    }
}
...
async function batchLinkProviderConfigs() {
    ...
    const providerSummary = Object.entries(groupedByProvider)
        .map(([name, count]) => t('upload.batchLinkSummary', { name, count }))
        .join(', ');
