...
import { showToast, apiRequest, bindOnce, escapeHtml } from './utils.js';

function getPluginId(plugin) {
    return plugin.id || plugin.name;
}

function getPluginDisplayName(pluginOrId) {
    const id = typeof pluginOrId === 'string' ? pluginOrId : getPluginId(pluginOrId);
    const key = `plugins.catalog.${id}.name`;
    const translated = t(key);
    if (translated !== key) return translated;
    if (typeof pluginOrId === 'string') return pluginOrId;
    return pluginOrId.name || id;
}

function getPluginDisplayDescriptionHtml(plugin) {
    const id = getPluginId(plugin);
    const key = `plugins.catalog.${id}.description`;
    const translated = t(key);
    if (translated !== key) return translated;
    const desc = plugin.description;
    if (!desc) return escapeHtml(t('plugins.noDescription'));
...
export function initPluginManager() {
    ...
    window.addEventListener('languageChanged', () => {
        if (pluginsList.length > 0) renderPluginsList();
        if (marketPluginsList.length > 0) renderMarketList();
    });
}
...
function renderPluginsList() {
    ...
    pluginsList.forEach(plugin => {
        ...
        let badgesHtml = '';
        if (plugin.hasMiddleware) badgesHtml += `<span class="plugin-badge middleware" title="${t('plugins.badge.middleware.title')}">${t('plugins.badge.middleware')}</span>`;
        if (plugin.hasRoutes) badgesHtml += `<span class="plugin-badge routes" title="${t('plugins.badge.routes.title')}">${t('plugins.badge.routes')}</span>`;
        if (plugin.hasHooks) badgesHtml += `<span class="plugin-badge hooks" title="${t('plugins.badge.hooks.title')}">${t('plugins.badge.hooks')}</span>`;
        
        card.innerHTML = `
            <div class="plugin-header">
                <div class="plugin-title">
                    <h3>${escapeHtml(getPluginDisplayName(plugin))}</h3>
                    <span class="plugin-version">v${plugin.version}</span>
                </div>
                <div class="plugin-actions" style="display: flex; align-items: center; gap: 10px;">
                    ${!plugin.isBuiltin ? `<button class="btn-uninstall-icon" title="${t('plugins.uninstallTitle')}" onclick="window.uninstallPlugin('${plugin.name}')" style="background: none; border: none; color: var(--danger-color); cursor: pointer; padding: 4px; display: inline-flex; align-items: center; justify-content: center; font-size: 1rem;"><i class="fas fa-trash-alt"></i></button>` : ''}
            ...
            <div class="plugin-description">${getPluginDisplayDescriptionHtml(plugin)}</div>
...
export async function loadMarketPlugins() {
    ...
    try {
        ...
        if (response && response.plugins) {
            ...
            renderMarketList();
            if (updateUrl) showToast(t('common.success'), t('plugins.market.indexUpdated'), 'success');
        } else {
            if (emptyEl) emptyEl.style.display = 'flex';
        }
    } catch (error) {
        console.error('Failed to load market plugins:', error);
        showToast(t('common.error'), t('plugins.market.loadFailed') + ': ' + error.message, 'error');
        if (emptyEl) emptyEl.style.display = 'flex';
    } finally {
        if (loadingEl) loadingEl.style.display = 'none';
    }
...
function renderMarketList() {
    ...
    marketPluginsList.forEach(plugin => {
        ...
        if (isInstalled && !hasUpdate) {
            actionButton = `<button class="btn btn-secondary btn-install" disabled><i class="fas fa-check"></i> ${t('plugins.market.installed')}</button>`;
        } else if (hasUpdate) {
            if (plugin.isPaid) {
            ...
            actionButton = `<button class="btn btn-secondary btn-install" disabled title="${t('plugins.market.versionIncompatible', { version: plugin.minSystemVersion })}"><i class="fas fa-exclamation-triangle"></i> ${t('plugins.market.incompatible')}</button>`;
        } else if (plugin.isPaid) {
            actionButton = `<button class="btn btn-primary btn-install" style="background: var(--warning-color); border-color: var(--warning-color)" onclick="window.showPayment('${plugin.id}')"><i class="fas fa-shopping-cart"></i> ${t('plugins.market.buy')}</button>`;
        } else {
            actionButton = `<button class="btn btn-primary btn-install" onclick="window.installPlugin('${plugin.id}')"><i class="fas fa-download"></i> ${t('plugins.install')}</button>`;
        }
        ...
        card.innerHTML = `
            <div class="plugin-header">
                <div class="plugin-title">
                    ...
                    ${plugin.isPaid ? `<span class="plugin-badge" style="background: var(--warning-bg); color: var(--warning-text); margin-top: 5px">${plugin.price || t('plugins.paid')}</span>` : ''}
                </div>
            </div>
            <div class="plugin-description">${getPluginDisplayDescriptionHtml(plugin)}</div>
...
export function showPayment(pluginId) {
    ...
    document.getElementById('paymentTitle').textContent = t('plugins.payment.buyTitle', { name: getPluginDisplayName(plugin) });
    document.getElementById('paymentPrice').textContent = plugin.price || t('plugins.paidPlugin');
    document.getElementById('paymentDesc').innerHTML = getPluginDisplayDescriptionHtml(plugin);
...
export async function installPlugin(pluginId) {
    ...
    try {
        ...
        if (response.success) {
            showToast(t('common.success'), t('plugins.installSuccess', { name: getPluginDisplayName(plugin) }), 'success');
            await loadPlugins();
            renderMarketList();
        }
    } catch (error) {
        console.error('Failed to install plugin:', error);
        showToast(t('common.error'), t('plugins.installFailed') + ': ' + error.message, 'error');
        overlay.remove();
    }
}
...
async function uploadAndInstallPlugin(file) {
    ...
    showToast(t('common.info'), t('plugins.payment.uploading'), 'info');
    ...
    try {
        ...
        if (result.success) {
            showToast(t('common.success'), t('plugins.payment.success'), 'success');
            document.getElementById('paymentModal').classList.remove('show');
            await loadPlugins();
            renderMarketList();
        } else {
            throw new Error(result.error?.message || t('plugins.installFailed'));
        }
    } catch (error) {
...
export async function togglePlugin(pluginName, enabled) {
    try {
        ...
        showToast(t('common.success'), t('plugins.toggle.success', { name: getPluginDisplayName(pluginName), status: enabled ? t('common.enabled') : t('common.disabled') }), 'success');
        loadPlugins();
...
export async function uninstallPlugin(pluginName) {
    const confirmed = confirm(t('plugins.uninstall.confirm', { name: getPluginDisplayName(pluginName) }));
    if (!confirmed) return;
    ...
    try {
        ...
        if (response && response.success) {
            showToast(t('common.success'), t('plugins.uninstall.success', { name: getPluginDisplayName(pluginName) }), 'success');
            await loadPlugins();
            if (typeof renderMarketList === 'function') {
                renderMarketList();
            } else {
            ...
            showToast(t('common.info'), t('plugins.restart.hint'), 'info');
        } else {
            throw new Error(response?.error?.message || t('plugins.uninstall.failed'));
        }
    } catch (error) {
        console.error(`Failed to uninstall plugin ${pluginName}:`, error);
        showToast(t('common.error'), t('plugins.uninstall.failed') + ': ' + error.message, 'error');
    }
}
