...
function buildCherryStudioSnippet(providerName, baseUrl, apiKey, model, format) {
    ...
    if (format === 'json') {
        return toPrettyJson({
            ...
            name: `${t('header.title')} (${providerName})`,
            baseUrl,
    ...
    if (format === 'env') {
        return buildEnvSnippet([
            ...
            ['CHERRY_STUDIO_NAME', `${t('header.title')} (${providerName})`],
    ...
    return buildMarkdownSnippet('Cherry Studio', [
        ...
        ['access.snippets.field.name', `${t('header.title')} (${providerName})`],
...
function buildClineSnippet(providerName, baseUrl, apiKey, model, format) {
    ...
    if (format === 'json') {
        return toPrettyJson({
            ...
            profileName: `${t('header.title')} (${providerName})`,
            baseUrl,
            apiKey: resolvedApiKey,
            modelId: model
        });
    ...
    if (format === 'env') {
        return buildEnvSnippet([
            ...
            ['CLINE_PROFILE_NAME', `${t('header.title')} (${providerName})`],
    ...
    return buildMarkdownSnippet('Cline', [
        ['access.snippets.field.provider', 'access.snippets.field.protocol'],
        ['access.snippets.field.profileName', `${t('header.title')} (${providerName})`],
        ['access.snippets.field.baseUrl', baseUrl],
        ['access.snippets.field.apiKey', resolvedApiKey],
