            ...
            <div class="provider-modal-body">
                <div class="provider-summary">
                    ...
                    <div class="provider-summary-actions">
                        ...
                        <button class="btn btn-warning" onclick="window.resetAllProvidersHealth('${providerType}')" data-i18n="modal.provider.resetHealth" title="${t('modal.provider.resetHealth')}">
                            <i class="fas fa-heartbeat"></i> ${t('modal.provider.resetHealth')}
                        </button>
                        <button class="btn btn-info" onclick="window.performHealthCheck('${providerType}')" data-i18n="modal.provider.healthCheck" title="${t('modal.provider.healthCheck')}">
                            <i class="fas fa-stethoscope"></i> ${t('modal.provider.healthCheck')}
                        </button>
                        <button class="btn btn-secondary" onclick="window.refreshUnhealthyUuids('${providerType}')" data-i18n="modal.provider.refreshUnhealthyUuids" title="${t('modal.provider.refreshUnhealthyUuids')}">
                        ...
                        <button class="btn btn-danger" onclick="window.deleteUnhealthyProviders('${providerType}')" data-i18n="modal.provider.deleteUnhealthy" title="${t('modal.provider.deleteUnhealthy')}">
                ...
                <div class="provider-nodes-toolbar" style="margin-bottom: 15px; display: flex; gap: 10px; align-items: center;">
                    <div class="search-input-wrapper" style="position: relative; flex: 1;">
                        ...
                        <input type="text" id="nodeSearchInput" 
                               ...
                               placeholder="${t('modal.provider.searchNodesPlaceholder')}" 
                    ...
                    <div class="view-mode-toggle" style="display: flex; background: var(--bg-secondary); padding: 4px; border-radius: 8px; border: 1px solid var(--border-color);">
                        <button class="view-mode-btn ${currentViewMode === 'list' ? 'active' : ''}" data-mode="list" title="${t('common.view.list')}" style="border: none; background: ${currentViewMode === 'list' ? 'var(--primary-color)' : 'transparent'}; color: ${currentViewMode === 'list' ? '#fff' : 'var(--text-secondary)'}; padding: 6px 10px; border-radius: 6px; cursor: pointer; transition: all 0.2s;">
                            <i class="fas fa-list"></i>
                        </button>
                        <button class="view-mode-btn ${currentViewMode === 'card' ? 'active' : ''}" data-mode="card" title="${t('common.view.card')}" style="border: none; background: ${currentViewMode === 'card' ? 'var(--primary-color)' : 'transparent'}; color: ${currentViewMode === 'card' ? '#fff' : 'var(--text-secondary)'}; padding: 6px 10px; border-radius: 6px; cursor: pointer; transition: all 0.2s;">
...
async function loadModelsForProviderType(providerType, providers) {
    try {
        ...
        providers.forEach(provider => {
            ...
            if (container) {
                container.innerHTML = `<div class="error-message">${t('common.error')}: ${t('modal.provider.loadModelsFailed')}</div>`;
            }
        });
    }
}
            ...
            <div class="provider-item-detail ${healthClass} ${disabledClass}" data-uuid="${provider.uuid}">
                <div class="provider-item-header" onclick="window.toggleProviderDetails('${provider.uuid}')">
                    ...
                    <div class="provider-actions-group">
                        <button class="btn-small ${toggleButtonClass}" onclick="window.toggleProviderStatus('${provider.uuid}', event)" title="${t('modal.provider.toggleTitle', { action: toggleButtonText })}">
...
function renderProviderConfig(provider) {
    ...
    baseFields.forEach(fieldKey => {
        ...
        const placeholder = fieldDef.placeholder || (fieldKey === 'customName' ? t('config.placeholder.nodeName') : (fieldKey === 'checkModelName' ? t('config.placeholder.model') : (fieldKey === 'concurrencyLimit' ? t('modal.provider.concurrencyPlaceholder') : (fieldKey === 'queueLimit' ? t('modal.provider.queuePlaceholder') : ''))));
                ...
                <div class="config-item">
                    ...
                    <div class="file-input-group">
                        <input type="text"
                               ...
                               data-config-value="${(field1Value !== undefined && field1Value !== null) ? field1Value : ''}"
                               placeholder="${field1Def.placeholder || ''}">
                       <button type="button" class="btn btn-outline upload-btn" data-target="edit-${provider.uuid}-${field1Key}" aria-label="${t('modal.uploadFile')}" disabled>
                    ...
                    <div class="config-item">
                        ...
                        <div class="file-input-group">
                            <input type="text"
                                   ...
                                   data-config-value="${(field2Value !== undefined && field2Value !== null) ? field2Value : ''}"
                                   placeholder="${field2Def.placeholder || ''}">
                            <button type="button" class="btn btn-outline upload-btn" data-target="edit-${provider.uuid}-${field2Key}" aria-label="${t('modal.uploadFile')}" disabled>
                                <i class="fas fa-upload"></i>
                            </button>
                        </div>
...
function cancelEdit(uuid, event) {
    ...
    actionsGroup.innerHTML = `
        <button class="btn-small ${toggleButtonClass}" onclick="window.toggleProviderStatus('${uuid}', event)" title="${t('modal.provider.toggleTitle', { action: toggleButtonText })}">
            <i class="${toggleButtonIcon}"></i> ${toggleButtonText}
        </button>
    ...
    form.innerHTML = `
        ...
        <div class="form-grid">
            <div class="form-group">
                ...
                <input type="text" id="newCustomName" data-i18n="modal.provider.customName" data-i18n-placeholder="config.placeholder.nodeName" placeholder="${t('config.placeholder.nodeName')}">
            </div>
            <div class="form-group">
                ...
                <input type="text" id="newCheckModelName" data-i18n="modal.provider.checkModelName" data-i18n-placeholder="config.placeholder.model" placeholder="${t('config.placeholder.model')}">
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <input type="number" id="newConcurrencyLimit" placeholder="${t('modal.provider.concurrencyPlaceholder')}">
            </div>
            <div class="form-group">
                ...
                <input type="number" id="newQueueLimit" placeholder="${t('modal.provider.queuePlaceholder')}">
            </div>
        </div>
    ...
    fields += `
        <div class="form-group">
            ...
            <div class="file-input-group">
                ...
                <button type="button" class="btn btn-outline upload-btn" data-target="new${field1.id}" aria-label="${t('modal.uploadFile')}">
                    <i class="fas fa-upload"></i>
                </button>
            </div>
    ...
    fields += `
        <div class="form-group">
            ...
            <div class="file-input-group">
                ...
                <button type="button" class="btn btn-outline upload-btn" data-target="new${field2.id}" aria-label="${t('modal.uploadFile')}">
                    <i class="fas fa-upload"></i>
                </button>
            </div>
            ${isKiroField ? '<small class="form-text"><i class="fas fa-info-circle"></i> ' + t('modal.provider.kiroAuthHint') + '</small>' : ''}
