...
async function handleSend() {
    ...
    const displayText = [
        text,
        ...filesToSend.map(f => t('playground.attachment', { name: f.name }))
    ].filter(Boolean).join('\n');
...
async function imageResponse(provider, model, prompt, files, bubble, interfaceType) {
    ...
    try {
        ...
        if (isEdit) {
            if (imageFiles.length === 0) throw new Error(t('playground.uploadImageFirst'));
...
async function unaryResponse(provider, model, bubble, params) {
    ...
    try {
        ...
        if (content) {
            ...
            const historyContent = content.replace(/data:[^;]+;base64,[A-Za-z0-9+/=]+/g, t('playground.imageTag'));
...
function appendMessage(role, text) {
    ...
    if (role === 'assistant') {
        ...
        actions.innerHTML = `
            <div class="pg-action-link btn-copy-msg" title="${t('playground.copyText')}">
        ...
        actions.className = 'pg-message-actions';
        actions.innerHTML = `
            <div class="pg-action-link btn-retry-msg" title="${t('playground.retryChat')}">
