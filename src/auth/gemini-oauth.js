...
import { getOAuthClientCredentials } from './oauth-client-credentials.js';

const geminiCliCreds = getOAuthClientCredentials('gemini-cli-oauth');
const antigravityCreds = getOAuthClientCredentials('gemini-antigravity');
...
const OAUTH_PROVIDERS = {
    'gemini-cli-oauth': {
        clientId: geminiCliCreds.clientId,
        clientSecret: geminiCliCreds.clientSecret,
        port: 8085,
    ...
    'gemini-antigravity': {
        clientId: antigravityCreds.clientId,
        clientSecret: antigravityCreds.clientSecret,
        port: 8086,
...
function generateResponsePage(isSuccess, message, provider = null) {
    const title = isSuccess ? 'Xác thực thành công!' : 'Xác thực thất bại';
    const countdownHtml = isSuccess ? `
        <p>Cửa sổ sẽ tự đóng sau <span id="countdown" style="font-weight: bold; color: #2196f3;">10</span> giây.</p>
        <script>
            const notifyOpener = () => {
                try {
            ...
            try {
                ...
                if (code) {
                    ...
                    try {
                        ...
                        res.end(generateResponsePage(true, 'Bạn có thể đóng trang này.', provider));
                    } catch (tokenError) {
                        ...
                        res.end(generateResponsePage(false, `Không lấy được token: ${tokenError.message}`, provider));
                    } finally {
                        server.close(() => {
                            activeServers.delete(provider);
                        });
                    ...
                    clearPollTimer();
                    const errorMessage = `Xác thực thất bại. Google trả về lỗi: ${errorParam}`;
                ...
                res.writeHead(500, { 'Content-Type': 'text/html; charset=utf-8' });
                res.end(generateResponsePage(false, `Lỗi máy chủ: ${error.message}`, provider));
