        ...
        return new Promise((resolve, reject) => {
            ...
            server.on('request', (req, res) => {
                ...
                if (code && state) {
                    ...
                    res.end(generateResponsePage(true, 'Xác thực Grok CLI thành công. Thông tin đăng nhập sẽ được lưu tự động.'));
                    server.emit('auth-success', { code, state });
                    return;
                }

                res.writeHead(400, { 'Content-Type': 'text/html; charset=utf-8' });
                res.end(generateResponsePage(false, 'Callback thiếu mã ủy quyền hoặc state.'));
            });
    ...
    try {
        ...
        return {
            ...
            authInfo: {
                ...
                instructions: [
                    '1. Nhấn nút bên dưới để mở liên kết ủy quyền trong trình duyệt',
                    '2. Đăng nhập bằng tài khoản xAI/Grok của bạn',
                    '3. Ủy quyền cho Grok CLI truy cập xAI API',
                    '4. Sau khi xác thực thành công, thông tin đăng nhập sẽ được lưu tự động',
                    '5. Nếu trình duyệt không tự chuyển hướng, hãy sao chép URL callback thủ công'
                ]
            }
        };
    } catch (error) {
