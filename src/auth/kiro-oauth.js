...
function generateResponsePage(isSuccess, message) {
    ...
    const countdownHtml = isSuccess ? `
        ...
        <p style="font-size: 12px; color: #999;">Mẹo: Nhấn F12 để xem nhật ký chi tiết.</p>
        <script>
            ...
            try {
                ...
                if (url.pathname === '/oauth/callback' || url.pathname === '/authenticate-success') {
                    ...
                    if (!tokenResponse.ok) {
                        ...
                        res.end(generateResponsePage(false, `Không lấy được token: ${tokenResponse.status}`));
                        return;
                    }
                    ...
                    res.end(generateResponsePage(true, 'Xác thực thành công! Bạn có thể đóng trang này.'));
