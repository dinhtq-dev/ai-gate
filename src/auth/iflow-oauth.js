            ...
            try {
                ...
                if (url.pathname === '/oauth2callback') {
                    ...
                    if (errorParam) {
                        ...
                        res.end(generateResponsePage(false, `Xác thực thất bại: ${errorParam}`));
                        server.close(() => {
                    ...
                    if (state !== expectedState) {
                        ...
                        res.writeHead(400, { 'Content-Type': 'text/html; charset=utf-8' });
                        res.end(generateResponsePage(false, 'Xác minh State thất bại'));
                        server.close(() => {
                            activeIFlowServers.delete('openai-iflow');
                        });
                        return;
                    ...
                    if (!code) {
                        ...
                        res.writeHead(400, { 'Content-Type': 'text/html; charset=utf-8' });
                        res.end(generateResponsePage(false, 'Thiếu mã ủy quyền'));
                        server.close(() => {
                            activeIFlowServers.delete('openai-iflow');
                        });
                        return;
                    ...
                    try {
                        ...
                        res.end(generateResponsePage(true, `Xác thực thành công! Tài khoản: ${userInfo.email}. Bạn có thể đóng trang này.`));
                        
                    } catch (tokenError) {
                        logger.error(`${IFLOW_OAUTH_CONFIG.logPrefix} 令牌处理失败:`, tokenError);
                        res.writeHead(500, { 'Content-Type': 'text/html; charset=utf-8' });
                        res.end(generateResponsePage(false, `Xử lý token thất bại: ${tokenError.message}`));
                    } finally {
                        server.close(() => {
                            activeIFlowServers.delete('openai-iflow');
                        });
                ...
                res.writeHead(500, { 'Content-Type': 'text/html; charset=utf-8' });
                res.end(generateResponsePage(false, `Lỗi máy chủ: ${error.message}`));
